...
Moreover, regarding the preprocessing, we tried to standarize the data by subtracking the mean value of each joint and also divided by standard deviation. One point worth mentioning is that some joints had zero standard deviation, meaning that the training data set were not changing at all at these angles, therefore we decided for angles with 0 std, not to use them at the analyis. This end up to an LSTM of 51 angles instead of 75. If the test set is coming from the same distribution, we could expect that the same angles also in the test set have 0 std so they can be removed fromt he LSTM  analysis and for the test purposes just keep the last frame of these angles as the predicted one.

Another preprocessing step would be to add gaussian noise to the angles. This would make the LSTM more robust and would help to avoid overfitting. The only disadvantage of that is that we add another parameter to tune, like the std of the gausian noise, but ont he other side it would make the training process more reliable, since the LSTM will not be that vulnerable to overfitting.

\includegraphics[width=5cm,height=5cm]{Prediction_achitecture.PNG}

\subsection{Network structure}
 In the following table the parameters of the LSTM architecture can be seen. The training process show us  that after 10 epochs there was not big decrease in validation error, therefore the final submission is with 9 epochs.
