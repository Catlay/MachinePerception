...
class RNNModel(object):
    ...
    def build_model(self):
        ...
        with tf.variable_scope('rnn_model', reuse=self.reuse):
            ...
            outputs_all_unroll=tf.reshape(outputs_all_stacked,[self.config['batch_size']*(self.max_seq_length),self.config['hidden_states']])
            print ('outputuunroll',outputs_all_unroll.get_shape())
        with tf.variable_scope('output', reuse=self.reuse): 
            W=tf.get_variable(name='W',shape=[self.config['hidden_states'],75],initializer=tf.contrib.layers.xavier_initializer())
            b= tf.get_variable(name='b',shape=[75],initializer=tf.contrib.layers.xavier_initializer())
            tf.get_variable_scope().reuse_variables()
      #  print(b.get_shape())
            print('hW',W.get_shape()) 
            logits=tf.matmul(outputs_all_unroll,W) + b
            self.predictions=tf.reshape(logits,[self.config['batch_size'],(self.max_seq_length),75])
            print('finished RNN')
          # predictions1= tf.argmax(logits,axis=0)
        
         ## print('logits',predictions1.get_shape())
    ...
    def get_feed_dict(self, batch):
        ...
        initial_state_=np.zeros([ len(input_padded ),3],dtype=float)
