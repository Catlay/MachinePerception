    ...
    def build_loss(self):
        ...
        if self.mode is not 'inference':
            with tf.name_scope('loss'):
                ...
                print(self.mask.get_shape())
                error= tf.abs((self.target-self.predictions) )
                print(' before change',error.get_shape())
                ...
                #mask=tf.unstack(self.mask)
               # print('unstacked mask is', mask.get_shape())
               # masknew=[]
               # for i in range(mask.get_shape[0]):
               #  masknew=tf.concat[masknew,tf.tile(mask[i],75)]
               # masknewreshape=tf.reshape(masknew, [self.mask.get_shape[0],self.mask.get_shape[0],75 ])
               # print(masknewreshape.get_shape())
